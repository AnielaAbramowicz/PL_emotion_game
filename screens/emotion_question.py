...
class EmotionQuestionScreen:
    ...
    def render(self):
        ...
        round_label = tk.Label(
            self.main_frame,
            text=f"Runda {self.gs.round + 1} / {self.gs.num_questions}",
        ...
        for idx, emo in enumerate(self.emotions):
            ...
            btn.bind("<Button-1>", lambda e, emotions=emo: self.check_answer(emotions))
            self.buttons.append(btn)


    def set_difficulty(self, level):
        self.gs.set_difficulty(level, manual = True)
        self.clear()
        self.render()
    ...
    def check_answer(self, selected):
        ...
        # with open("questions_times.txt", "a") as f:
        #     f.write(f"{time_spent:.2f} seconds, correctness: {correctness}\n")

        self.show_feedback(correctness, correct_label.capitalize())
        print("correct label")
        print(correct_label)
        print(correct_label.capitalize())
        print("correctness")
        print(correctness)

        print(f"selected=<{selected}>")
        print(f"correct_label=<{correct_label}>")
        print(f"selected == correct_label? {selected == correct_label}")
        print(f"selected repr: {repr(selected)}")
        print(f"correct_label repr: {repr(correct_label)}")
    ...
    def show_feedback(self, correctness, correct_answer):
        ...
        btn.bind("<Button-1>", lambda e: self. proceed_after_feedback())
        btn.pack(pady=5)
