...
class ReasonFeedbackScreen:
    ...
    def log_and_continue(self, answer_number):
        ...
        difficulty_norm = self.gs.last_difficulty 
        ...
        if answer_number == 2:
            last_emotion = self.gs.df.iloc[self.gs.current_index]["folder_label"].title()
            with open(os.path.join(self.gs.session_folder, "reason_answers.txt"), "a") as f:
                f.write(f"{total_time:.2f} seconds - Answer: {text}- Category: {last_emotion}\n")
            self.gs.emotion_suppression[last_emotion] = 1

        else:
            with open(os.path.join(self.gs.session_folder, "reason_answers.txt"), "a") as f:
                f.write(f"{total_time:.2f} seconds - Answer: {text}- Category: GAME\n")
        ...
        self.gs.update_emotion_probabilities()
